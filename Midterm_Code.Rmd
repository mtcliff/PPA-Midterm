...
houses.model <- houses %>% dplyr::filter(toPredict == "MODELLING")
...
```{r hood_MAPE, warning=FALSE}

st_drop_geometry(houses.model.test) %>%
  #filter(sale_price.APE < 127) %>%
  group_by(NAME) %>%
  ...
  left_join(hoods, by = c("NAME" = "NAME")) %>%
    ...
    ggplot() + 
      ...
      #geom_sf(data = houses.model.test, colour = "black", size = .5) +
      scale_fill_gradient(low = palette5[1], high = palette5[5],
                          name = "MAPE") +
      labs(title = "Mean test set MAPE by neighborhood") +
      mapTheme()

outlier <- houses.model.test %>% filter(sale_price.Predict < 0)

```

```{r income_context, warning=FALSE}
